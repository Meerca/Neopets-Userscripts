...
(function() {
    ...
    class BargainStocksListElement extends HTMLElement {
        ...
        _updateUI() {
            const stocks = findCheapestPurchaseableStocks(this.stocksList, this.minPrice, this.maxPrice);
        ...
        _handleMinPriceConfig() {
            ...
            let newMin
            do {
                newMin = prompt(newMin ? `Please specify a numeric value.\n${promptMessage}` : promptMessage, this.minPrice)
                if (!newMin) {
                    return;
                }
        ...
        _setContent(stocks) {
            ...
            this.message.textContent = `The following ${stocks.length} ${pluralize(stocks.length, "stock")} are available to purchase at or near ${this.minPrice} NP.`
             + " Click the stock ticker to go to the purchase page.";

            this.list.append(...stocks.map(({ ticker, price }) => {
    ...
    function findCheapestPurchaseableStocks(stocksMap, minPrice = 15, maxPrice = minPrice + 5) {
        let cheapest = [];
